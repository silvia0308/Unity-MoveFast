...
public class DataCollector : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (conductColletion)
        {
            ...
            headset.position = new Position(transform.GetChild(1).position);
            headset.rotation = new Rotation(transform.GetChild(1).rotation);
            dataInfo.headset = headset;
            Device leftController = new Device();
            leftController.position = new Position(transform.GetChild(4).position);
            leftController.rotation = new Rotation(transform.GetChild(4).rotation);
            dataInfo.leftController = leftController;
            Device rightController = new Device();
            rightController.position = new Position(transform.GetChild(5).position);
            rightController.rotation = new Rotation(transform.GetChild(5).rotation);
